...
PYBIND11_MODULE(pyqreach, m) {
    ...
    py::class_<QOperation>(m, "QOperation")
        ...
        .def("post_image", &QOperation::postImage, py::arg("other"), "Return the post-image under a gate/projection QOperation")
        .def("dim", [](const QOperation& op) { return static_cast<int>(op.oplist.size()); }, "Return the number of terms in the QOperation")
        .def("is_zero", &QOperation::isZeroValue, "Return whether this QOperation has no nonzero terms")
